...
fils <- c(
  "~/Dropbox/Data/SREBP_SCAP/Genesis_Project/Genesis_Microscopy/ANG260/ANG260_Imgs/ANG260_Ni_20x_2021_12_10/20x/ANG260_20x_Well_005/ANG260_20x_Well_005_F_1/ANG260_20x_Well_005a_GFP.tif",
  "~/Dropbox/Data/SREBP_SCAP/Genesis_Project/Genesis_Microscopy/ANG260/ANG260_Imgs/ANG260_Ni_20x_2021_12_10/20x/ANG260_20x_Well_005/ANG260_20x_Well_005_F_1/ANG260_20x_Well_005a_RFP.tif"
)
```
...
```{r}
plot(as.cimg(EBImage::abind(masked, along=1)))
```

Generate a series of red channel images for Greg:
```{r}
mask.series <- lapply(c(0, 30, 50, 70), function(pc, img) {
  qtile.threshold <- pc/100
  qtile <- quantile(img, qtile.threshold)
  img[img < qtile] <- NA
  img
}, img = imgs.rescale[[2]])
plot(as.cimg(EBImage::abind(mask.series, along=1)))
```
```{r}
# getwd()
# lapply(mask.series, class)
for (i in seq_along(mask.series)) {
  out.file <- paste0("red_channel_", i, ".png")
  print(out.file)
  img <- mask.series[[i]]
  img[is.na(img)] <- 1
  imager::save.image(img, out.file)
}
```
...
* in each masked crop set NA pixels to zero  
...
# lapply(masked.crop2, class) # "cimg"         "imager_array" "numeric"
plot(as.cimg(EBImage::abind(masked.crop2, along=1)))
```
```{r}
cor(masked.crop2[[1]], masked.crop2[[2]], use="pairwise.complete.obs")
```

OR-combine masks:
```{r}
mask.combined.OR <- as.matrix(masked.crop2[[1]] > 0 | masked.crop2[[2]] > 0)
class(mask.combined.OR) # "matrix" "array" 
dim(mask.combined.OR) # 600 600
typeof(mask.combined.OR) # "logical"
```

```{r}
imgs.crop.mask.or <- lapply(masked.crop2, function(im) {
  a <- im[,]
  a[!mask.combined.OR] <- NA
  as.cimg(a)
})
plot(as.cimg(EBImage::abind(imgs.crop.mask.or, along=1)))
```
```{r}
sum(imgs.crop.mask.or[[1]] == 0, na.rm=TRUE)
sum(imgs.crop.mask.or[[2]] == 0, na.rm=TRUE)
sum(imgs.crop.mask.or[[1]] == 0 & imgs.crop.mask.or[[2]] == 0, na.rm=TRUE)
```


```{r}
cor(imgs.crop.mask.or[[1]], imgs.crop.mask.or[[2]], use="pairwise.complete.obs")
```
